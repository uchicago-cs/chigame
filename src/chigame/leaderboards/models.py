...
class LeaderboardPrivacySetting(models.Model):
    ...
    def __str__(self):
        if self.leaderboard:
            return f"{self.user.username}'s settings for {self.leaderboard.name}"
        elif self.game:
            return f"{self.user.username}'s settings for {self.game.name}"
        else:
            return f"{self.user.username}'s global settings"
